...
const NavBar = () => {
  useGSAP(() => {
    gsap.to('#Navbar_Mercedes', { opacity: 1, y: 0, duration: 0.8, ease: 'power4.out' });
  }, [])
...
// Attach event listener with passive mode for better performance
  ...
  return (
    <>
      <header
          className={`navbar w-full fixed top-0 z-20 bg-white dark:bg-slate-900 shadow-md transition-transform duration-300 ${
              isVisible ? "-translate-y-px" : "-translate-y-full"
          }`}
      >
        <nav
            id="Navbar"
            className="max-w-[1550px] mx-auto flex items-center justify-between px-4 sm:px-16 py-5 relative"
        >
          <button
