...
const ProductInfo = ({ product, className }: ProductInfoProps) => {
    ...
    return (
        <div className={cn("flex flex-col", className)}>
            <h1 className="text-xl md:text-2xl font-semibold text-ali-text mb-2 dark:text-white">
                {product.name}
            </h1>
            ...
            <div className="flex items-center gap-4 mb-4 flex-wrap">
                ...
                <span className="text-ali-text-light text-sm dark:text-white">{product.orders} Orders</span>
            </div>
                ...
                <div className="flex items-baseline gap-2 mb-1">
                    {product.discountPrice ? (
                        <>
                            ...
                            <Badge className="bg-ali-red text-white ml-2">{discountPercentage}% OFF</Badge>
                        </>
                    ) : (
                        <span className="text-ali-red text-2xl font-bold">${product.price.toFixed(2)}</span>
                    )}
                </div>
            </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {product.colors.map((color) => (
                            <button
                                key={color.id}
                                className={cn(
                                    "p-0.5 rounded-md transition-all w-14 h-14 flex items-center justify-center border-2 dark:border-0",
                                    selectedColor?.id === color.id ? "border-ali-red" : "border-ali-border"
                                )}
                                onClick={() => setSelectedColor(color)}
                                title={color.name}
                            >
                                <div className="h-full w-full rounded" style={{ backgroundColor: color.code }}>
            ...
            <div className="mb-6">
                ...
                <div className="flex items-center">
                    <button
                        className="w-8 h-8 border border-ali-border rounded-l-md flex items-center justify-center bg-ali-light-gray text-black dark:text-white"
                    ...
                    <input
                        type="number"
                        className="w-16 h-8 border-y text-center outline-none text-ali-text text-black dark:text-black"
                        value={quantity}
                    ...
                    />
                    <button
                        className="w-8 h-8 border border-ali-border rounded-r-md flex items-center justify-center bg-ali-light-gray text-black dark:text-white"
                        onClick={() => handleQuantityChange(quantity + 1)}
            ...
            <div className="mb-6">
                ...
                <div className="flex flex-col gap-2">
                    {product.shipping.map((option, index) => (
                        <div key={index} className="flex items-center text-sm p-2 rounded-md">
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-6">
                <Button className="flex-1 bg-amber-600 text-white dark:text-white" size="lg" onClick={handleBuyNow}>
                    Buy Now
                </Button>
                <Button className="flex-1 bg-white text-ali-orange border border-ali-orange hover:bg-ali-light-gray dark:text-black" size="lg" onClick={handleAddToCart}>
                    <ShoppingCart className="mr-2 h-4 w-4" /> Add to Cart
                </Button>
                <Button variant="outline" size="icon" className="border-ali-border">
